...
df = df    [df.TOTAL_COST.notnull()]    .rename(columns=cols)    .drop(dropCols, axis=1)    .fillna('')
...
df.loc[:,['awardNoticeDate','budgetStart','budgetEnd','projectStart','projectEnd']] =     df.loc[:,['awardNoticeDate','budgetStart','budgetEnd','projectStart','projectEnd']]        .apply(lambda col: pd.to_datetime(col, errors='coerce'))
...
dfLinks = dfLinks    .merge(dfGrantLink.reset_index(), how='left', left_on='PROJECT_NUMBER', right_on='coreProjectNum')    .drop('coreProjectNum', axis=1)    .rename(columns={0:'projects'})
